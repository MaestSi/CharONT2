...
Obtain_preliminary_consensus <- function(fastq_file, TRC, max_num_reads_clustering, THR, PLUR, num_threads) {
  ...
  sample_name <- basename(gsub(pattern = "\\.fasta", replacement = "", x = fasta_file))
  sample_dir <- gsub(pattern = "inSilicoPCR", replacement = "preliminaryConsensusCalling", x = gsub(pattern = "\\.fasta", replacement = "", x = fasta_file))
