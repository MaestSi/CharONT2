...
Cluster_reads <- function(fastq_file, first_allele_preliminary, IQR_outliers_coef_precl, IQR_outliers_coef, min_clipped_len, sd_noise_score, num_alleles) {
  ...
  sample_name <- basename(gsub(pattern = "\\.fastq", replacement = "", x = fastq_file))
  sample_dir <- gsub(pattern = "inSilicoPCR", replacement = "readsAssignment", x = gsub(pattern = "\\.fastq", replacement = "", x = fastq_file))
